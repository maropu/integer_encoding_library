...
VSEncoding::VSEncoding(const uint32_t *lens,
                const uint32_t *zlens, uint32_t size,
                bool is_aligned) : is_aligned(is_aligned),
                        poss_sz(size), maxBlk(lens[size - 1]),
...
VSEncoding::compute_OptPartition(vector<uint32_t>& seq,
                uint32_t fixCost, vector<uint32_t>& parts) const
{
        __assert(seq.size() != 0 && seq.size() <= MAXLEN);
