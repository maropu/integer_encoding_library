...
uint32_t
OPTPForDelta::tryB(uint32_t b, uint32_t *in, uint32_t len) const
{
        __assert(b <= 32);
        __assert(in != NULL);
...
uint32_t
OPTPForDelta::findBestB(uint32_t *in, uint32_t len) const
{
        __assert(in != NULL);
        __assert(len != 0 && len <= MAXLEN);
                ...
                uint32_t *out, uint32_t &nvalue) const
{
        ...
        *out++ = numBlocks;
        ...
        for (uint32_t i = 0; i < numBlocks; i++) {
                if (i != numBlocks - 1) {
                        PForDelta::encodeBlock(in, PFORDELTA_BLOCKSZ, out, csize);
                        in += PFORDELTA_BLOCKSZ; 
                        out += csize;
                } else {
                        if ((len % PFORDELTA_BLOCKSZ) != 0)
                                PForDelta::encodeBlock(
                                                in, len % PFORDELTA_BLOCKSZ, out, csize);
                        else
                                PForDelta::encodeBlock(in, PFORDELTA_BLOCKSZ, out, csize);
                }
