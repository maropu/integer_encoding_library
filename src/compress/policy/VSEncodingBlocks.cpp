...
VSEncodingBlocks::encodeVS(uint32_t len,
                uint32_t *in, uint32_t &size, uint32_t *out) const
{
...
VSEncodingBlocks::decodeVS(uint32_t len,
                uint32_t *in, uint32_t *out, uint32_t *aux) const
{ 
        __assert(len != 0 && len <= MAXLEN);
        __assert(in != NULL);
        __assert(out != NULL);
        ...
        while  (1) {
                ...
                if (B) {
                         ...
                         * __simd_copy8() just copies 8 values each.
                         */
                        __simd_copy8(pblk[B], out);
                        __simd_copy8(pblk[B] + 8, out + 8);
                ...
                uint32_t len, uint32_t *out, uint32_t nvalue) const
{
        ...
        for (res = nvalue; res > VSENCODING_BLOCKSZ && in < iterm;
                        ...
                        res -= VSENCODING_BLOCKSZ) {
                sum = *in++;
                decodeVS(VSENCODING_BLOCKSZ, in, out, wmem_aux.get());
        }
