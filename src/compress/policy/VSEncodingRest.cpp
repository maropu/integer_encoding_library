...
typedef void (*__vserest_unpacker)(uint32_t **out,
                uint32_t **in, uint32_t &Fill, uint64_t &buffer);
                ...
                uint32_t *out, uint32_t &nvalue) const
{
        ...
        vdp->compute_OptPartition(logs,
                        VSEREST_LOGLEN + VSEREST_LOGLOG, parts);
